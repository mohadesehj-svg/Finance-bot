...
# آدرس وب‌هوک رندر شما (بسیار مهم برای بالا آمدن ربات)
RENDER_URL = "https://finance-bot-b418.onrender.com" 
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        ...
        await update.callback_query.message.reply_text(msg_text, reply_markup=InlineKeyboardMarkup(kb))
    else:
        await update.effective_message.reply_text(msg_text, reply_markup=InlineKeyboardMarkup(kb))
...
async def mat_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ خطا در ذخیره اطلاعات در دیتابیس سوپابیس. مطمئن شو جدول materials ساخته شده.")
...
# ═══════════════════════════════════════════════════════════
#  راه‌اندازی هماهنگ با رندر (Webhook)
# ═══════════════════════════════════════════════════════════
def main():
    app = Application.builder().token(BOT_TOKEN).build()

    # ایجاد تمامی هندلرهای مکالمه
    ...
    # 🚀 بخش حیاتی: تنظیم وب‌هوک متناسب با پورت رندر
